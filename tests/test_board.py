...
class TestBoardProperties:
    ...
    @pytest.mark.parametrize(
        "test_input", [(0, 0), (1, 0), (0, 1), (1, 1), (0, 2), (1, 2)]
    )
    ...
    def test__str__empty_board(self, board_sizes: tuple[int, int], expected: str):
        ...
        board = Board(
            dim_x=board_sizes[0],
            dim_y=board_sizes[1],
            red_conn_comp=fake_uf,
            blue_conn_comp=fake_uf,
        )
        print(str(board))
        print(expected)
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param("small", (0, 0), {Cell(0, 1), Cell(1, 0), Cell(1, 1)}),
            pytest.param("small", (1, 0), {Cell(0, 0), Cell(1, 1)}),
            pytest.param(
                "small", (0, 1), {Cell(0, 0), Cell(1, 1), Cell(1, 2), Cell(0, 2)}
            ),
            pytest.param(
                "medium",
                (1, 1),
    ...
    def test_neighbours_of_cell(
        self, board_size: str, tile: tuple[int, int], expected: set[tuple[int, int]]
    ):
        if board_size == "small":
            dim_x = 2
            dim_y = 3
    ...
    @pytest.mark.parametrize(
        "color_str, expected",
        [pytest.param("red", ([], [])), pytest.param("blue", ([], []))],
    )
